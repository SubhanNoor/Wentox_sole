...
export default function GroupAcSetupPage() {
  ...
  const filteredGroups = useMemo(() => {
    if (!searchQuery.trim()) return state.groupAccounts;
    const q = searchQuery.toLowerCase();
    return state.groupAccounts.filter(g => 
      g.name.toLowerCase().includes(q) || 
      g.id.toLowerCase().includes(q) ||
      g.class.toLowerCase().includes(q)
    );
  }, [state.groupAccounts, searchQuery]);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredGroups.map(grp => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        {/* Card Top: Code & Account Class badge */}
