...
import { Plus, Search, Settings, Save, Edit2, RotateCcw, Trash2, X, Landmark, XOctagon } from 'lucide-react';
...
export default function BusinessAcSetupPage() {
  ...
  // Once a parent chart account has been picked for this modal session, keyboard focus skips
  // straight to Account Name on every subsequent creation — only resets when the modal closes.
  // The field itself stays enabled: a deliberate click can still switch to a different chart
  ...
  // this page used to always fetch AND display every closed account inline with a badge, with no
  // way to hide them at all (BankSetupPage already had this toggle; Business/Chart/Group did not).
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (selectedId) {
      ...
      setCityId('');
      setOpeningBalance('');
      // Chart A/C is locked in after the first save of this modal session — Enter/Tab now goes
      // straight to Account Name, skipping the chart field, until the modal is closed.
  ...
  // (businessAccounts.service.js#remove already enforces the reserved/party-linked/has-
  // transactions guards; this is just the confirmation UI in front of it). Password-gated per the
  // user (2026-09-17), same as every other destructive delete in the app — PasswordPromptModal
  // IS the confirmation step now, replacing the old plain ConfirmModal.
  ...
  const confirmDeleteBusinessAc = async (password: string) => {
    ...
    setSuccessMsg('Business Account deleted.');
    setTimeout(() => setSuccessMsg(''), 3000);
    setDeletingBiz(null);
    await loadData();
  };
  ...
  // (2026-09-17). Only reachable for an already-closed account (see the CLOSED-only button below);
  // the backend's hasAnyReference() check is the real guard — this is irreversible so the wording
  // has to say so plainly, unlike the soft-delete modal above.
  const [permDeletingBiz, setPermDeletingBiz] = useState<BusinessAccountRow | null>(null);
  const confirmPermanentDeleteBusinessAc = async (password: string) => {
    if (!permDeletingBiz) return;
    const res = await businessAccountsApi.permanentDelete(permDeletingBiz.ba_id, password);
    if (!res.ok) {
      setErrorMsg(res.error.message);
      setTimeout(() => setErrorMsg(''), 5000);
      setPermDeletingBiz(null);
      return;
    }
    setSuccessMsg('Business Account permanently deleted.');
    setTimeout(() => setSuccessMsg(''), 3000);
    setPermDeletingBiz(null);
    await loadData();
  };
  ...
  }, [accounts, searchQuery, sortBy, selectedChartFilter, showInactive]);
  ...
  return (
    <AppLayout pageTitle="Business Accounts Setup">
      <div className="mx-auto" style={{ maxWidth: 1750 }}>
              ...
              <>
                ...
                {biz.status === 'CLOSED' ? (
                  <>
                    ...
                    <button
                      onClick={() => setPermDeletingBiz(biz)}
                      disabled={biz.is_reserved || biz.is_party_linked}
                      ...
                      title={
                        biz.is_reserved
                          ...
                          : biz.is_party_linked
                            ? 'Linked to a vendor, customer, employee, or bank — manage it from that setup screen instead'
        ...
        <PasswordPromptModal
          isOpen={!!permDeletingBiz}
          onClose={() => setPermDeletingBiz(null)}
          onSuccess={confirmPermanentDeleteBusinessAc}
          title="Permanently Delete Business Account"
          subtitle={permDeletingBiz ? `Confirm your password to PERMANENTLY delete "${permDeletingBiz.name}". This cannot be undone — the record itself is removed, not just closed. It will be refused if this account is still referenced anywhere (a vendor/customer/employee link, a posted or draft document, a settlement, transfer, or cheque allocation).` : undefined}
        />
