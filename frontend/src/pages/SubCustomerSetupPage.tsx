...
export default function SubCustomerSetupPage() {
  ...
  const handleSelectSubCustomer = (sub: { id: string; name: string; customerId: string }) => {
  ...
  const handleSaveSubCustomer = (e: React.FormEvent) => {
    ...
    if (selectedSubId) {
      ...
      dispatch({
        ...
        subCust: { id: selectedSubId, name: subName.trim(), customerId }
      });
      ...
      dispatch({
        ...
        subCust: { id: newId, name: subName.trim(), customerId }
      });
  ...
  const filteredSubCustomers = useMemo(() => {
    const list = state.subCustomers.map(sc => {
      const parentCustName = state.customers.find(c => c.id === sc.customerId)?.name || 'Unknown Parent';
      return { ...sc, parentCustName };
    });
    ...
    return list.filter(sc => 
      ...
      sc.parentCustName.toLowerCase().includes(q)
    );
  }, [state.subCustomers, state.customers, searchQuery]);
          ...
          <div className="mb-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Search and manage sub customers/delivery agents mapped to primary parent accounts.</p>
              </div>
              
              <div className="relative min-w-[265px]">
                <input
                  type="text"
                  placeholder="Search by sub name, parent customer..."
                  value={searchQuery}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredSubCustomers.map(sub => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        {/* Card Top: Code & Parent Customer badge */}
                        <div className="flex items-center justify-between mb-3.5 gap-2">
                          ...
                          <span className="text-[10px] font-bold text-[#B08D57] uppercase tracking-wider truncate max-w-[130px]" title={sub.parentCustName}>
                            {sub.parentCustName}
                          </span>
                        </div>
          ...
          <div className="card-white p-6 md:p-8 bg-white border">
            <div className="flex items-center gap-2 border-b pb-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Link a sub customer agent or sub destination account to a parent distributor customer.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveSubCustomer} className="flex flex-col gap-6">
              ...
              <div className="p-4 bg-slate-50 rounded-xl border flex flex-col gap-4" style={{ borderColor: 'var(--border-color)' }}>
                <div className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5 border-b pb-2">
                  <Settings size={15} className="text-[#B08D57]" /> Mapping Configuration
                </div>
