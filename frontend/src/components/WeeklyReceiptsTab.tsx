...
interface WeeklyReceiptsTabProps {
...
export default function WeeklyReceiptsTab({ onVoucherUnposted, onSettlementUnposted }: WeeklyReceiptsTabProps) {
  ...
  // memo rather than folded into `weeklyReceipts` since a settlement isn't a `ReceiptRow` and has
  // its own two-sided name (`from_name`/`to_name`) to search against.
  const weeklySettlements = useMemo(() => {
    return settlements.filter(s => {
      if (selectedMonth !== 'all') {
        const sMonth = new Date(s.settlement_date).getMonth().toString();
  ...
  const recordGroups = useMemo((): RecordGroup[] => {
    const combined: RecordGroup[] = [
      ...
      ...weeklySettlements.map((s): RecordGroup => ({ kind: 'settlement', settlement: s, totalAmount: s.amount })),
    ];
    return combined.sort((a, b) => b.totalAmount - a.totalAmount);
  }, [voucherCardsData, weeklySettlements]);
  ...
  const activeSettlement = useMemo(() => {
    ...
    return weeklySettlements.find(s => s.settlement_id === selectedSettlementId) ?? null;
  }, [selectedSettlementId, weeklySettlements]);
      ...
      <div className="card-white p-6 bg-white border border-slate-200 shadow-sm rounded-xl animate-in fade-in slide-in-from-bottom-3 duration-300">
        ...
        <div className="flex items-center justify-between border-b pb-4 mb-4" style={{ borderColor: 'var(--border-color)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-lora font-bold text-lg text-slate-800">
                Voucher #{activeVoucherHeader?.voucher_no ?? selectedVoucherId} — Weekly Receipts
              </h3>
              <p className="text-xs text-slate-500 font-medium">
    ...
    <div className="mx-auto" style={{ maxWidth: 1750 }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl border mb-6 bg-white shadow-2xs" style={{ borderColor: 'var(--border-color)' }}>
        ...
        <div className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-2 rounded-xl border border-slate-200">
          {weeklyReceipts.length} Receipt Record{weeklyReceipts.length === 1 ? '' : 's'}
          {weeklySettlements.length > 0 && `, ${weeklySettlements.length} Settlement${weeklySettlements.length === 1 ? '' : 's'}`}
        </div>
      </div>
            ...
            {recordGroups.length === 0 ? (
              <tr>
                <td colSpan={5} className="text-center p-12 text-slate-400">
                  ...
                  <p className="font-lora text-base font-semibold text-slate-500 mb-1">No Weekly Receipts Found</p>
                  <p className="text-xs max-w-sm mx-auto">No receipts were logged for this week matching your filters.</p>
                </td>
              </tr>
            ) : (
