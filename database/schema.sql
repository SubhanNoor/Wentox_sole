...
CREATE TABLE dbo.receipts (
  ...
  -- 028: ONLINE alternative to bank_id — settles against ANY business account, not only a bank.
  -- Never both; see CK_receipts_bank below. Existing rows keep using bank_id exactly as before.
  online_ba_id INT           NULL,
  ...
  CONSTRAINT FK_receipts_online_ba FOREIGN KEY (online_ba_id) REFERENCES dbo.business_accounts(ba_id),
  ...
  -- 028: ONLINE accepts bank_id OR online_ba_id (either is enough, never both); any other mode
  -- carries neither.
...
CREATE TABLE dbo.draft_receipts (
  ...
  -- 028: ONLINE alternative to bank_id, same as receipts.online_ba_id — no CHECK relax needed
  -- here, draft_receipts never had one requiring bank_id in the first place.
  online_ba_id INT           NULL,
  ...
  CONSTRAINT FK_draft_receipts_online_ba FOREIGN KEY (online_ba_id) REFERENCES dbo.business_accounts(ba_id),
...
CREATE TABLE dbo.expenses (
  ...
  -- 028/029: ONLINE alternative to bank_id — settles against ANY business account, not only a
  -- bank. Never both; see CK_expenses_payment below.
  online_ba_id INT           NULL,
  ...
  CONSTRAINT FK_expenses_online_ba FOREIGN KEY (online_ba_id) REFERENCES dbo.business_accounts(ba_id),
  ...
  -- Each mode carries exactly the identity it needs and nothing it does not. 029: ONLINE accepts
  -- bank_id OR online_ba_id (never both) instead of hard-requiring bank_id.
...
CREATE TABLE dbo.draft_expenses (
  ...
  -- 028/029: ONLINE alternative to bank_id, same as expenses.online_ba_id.
  online_ba_id INT           NULL,
  ...
  CONSTRAINT FK_draft_expenses_online_ba FOREIGN KEY (online_ba_id) REFERENCES dbo.business_accounts(ba_id),
  ...
  CONSTRAINT CK_draft_expenses_mode   CHECK (payment_mode IN
        ('CASH','CHEQUE_ENDORSED','CHEQUE_ISSUED','ONLINE')),
  -- 029: ONLINE accepts bank_id OR online_ba_id (never both), matching CK_expenses_payment.
...
/* ----------------------------------------------------------------------------
   dbo.stock_vouchers / dbo.stock_voucher_lines
   WHAT:  A manual "add stock" document (legacy Journal Entry-style
          bound-record screen) — N lines, each a finished-goods variant +
          cartons/pairs, entered under one Date/Store/Remarks header.
          Replaces the old inline "+ Add Stock" flow on the Current Stock
          report, which called stock:log-production and recorded every
          manual addition AS production — this is its own document type
          instead, same architecture as Journal Voucher: one table, DRAFT
          by default, status flips to CONFIRMED only on post().
   WHY:   Store is header-only/informational — dbo.stock_movements has no
          store_id column and stays store-agnostic; nothing downstream
          depends on store_id being set or accurate.
   POSTS AS: one dbo.stock_movements row per line, movement_type='ADJUSTMENT'
          (already unconstrained-sign), qty_pairs=+pairs,
          source_type='STOCK_VOUCHER', source_id=stock_voucher_id.
          unpost() deletes those same rows by source_type+source_id,
          mirroring how SALE_BILL/SALE_RETURN movements are looked up.
---------------------------------------------------------------------------- */
