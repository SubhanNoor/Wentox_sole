...
- [ ] `customers` (ipc handler/service/repository) CRUD (links to chart account `ac_id` + city, per UC-09 auto-creates a chart account exactly as UC-08 does for vendors)
- [ ] List endpoint returns linked account + city names

## Module 7.3 — Sub-Customers (UC-10)
- [ ] `subCustomers` (ipc handler/service/repository) CRUD (must belong to a customer)
- [ ] `customers:sub-customers` channel for the Sale Bill inline "+ Add Sub-Customer" flow
