...
- [ ] `cities` (ipc handler/service/repository) CRUD + unique-name validation
- [ ] `regions` (ipc handler/service/repository) CRUD
- [ ] `stores` (ipc handler/service/repository) CRUD
- [ ] `addas` (ipc handler/service/repository) CRUD; delete guard: block deletion when the adda is referenced by any sale bill (409) — `adda_id` is `NOT NULL` on sale bills in v4.3, so every bill has one
