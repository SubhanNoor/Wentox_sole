...
- [ ] `stock` (ipc handler/service/repository) — `stock:log-production` (PRODUCTION movement: input_qty + input_unit CARTONS/PAIRS, packing snapshot, qty_pairs normalized in the main process)
- [ ] `stock:adjust` — OPENING/ADJUSTMENT movements, signed qty
- [ ] `stock:movements` (payload: `{ product_id }`) — movement history
- [ ] `reports:stock` — current stock per product: `SUM(qty_pairs)`, displayed as cartons + extra pairs via packing (Current Stock tab)
- [ ] `reports:production` — production logs (PRODUCTION movements) with daily/weekly/monthly/overall date filters + article/category search
...
- [ ] `sale-bills:bilty-search` — filters: date range, customer, sub-customer, bill no (no "Without Bilty/Adda" option — every bill has both from creation)
- [ ] `sale-bills:update-bilty` — update bilty_no + adda_id (allowed on POSTED bills; non-financial)
