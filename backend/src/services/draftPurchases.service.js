...
// as if the purchase never happened. Also records the draft's system_no as permanently deleted
// (migration 032) — see draftSaleBills.service.js#remove()'s comment.
async function remove(draftId, userId) {
  ...
  await withTransaction(async (transaction) => {
    ...
    await deletedNumbersRepository.record(transaction, 'PURCHASE', draft.system_no, userId);
  });
  return { ok: true };
}
...
function listDeletedNumbers() {
  return deletedNumbersRepository.listByType('PURCHASE');
}
