...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_cheque_allocations_bank')
  ALTER TABLE dbo.cheque_allocations
    ADD CONSTRAINT FK_cheque_allocations_bank FOREIGN KEY (bank_id) REFERENCES dbo.bank_accounts(bank_id);
GO

-- Backfill every existing DEPOSIT allocation from the cheque it belongs to — the only bank any
-- deposit could ever have gone to before this migration, so this is exact, not a guess.
UPDATE ca
...
-- VENDOR_PAYMENT/EXPENSE_PAYMENT already require target_vendor_id/target_ba_id.
...
ALTER TABLE dbo.cheque_allocations ADD CONSTRAINT CK_cheque_allocations_target CHECK (
