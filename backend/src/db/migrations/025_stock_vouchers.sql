...
--
-- IF NOT EXISTS-guarded (matching 021/022's own pattern for the same reason): on a brand-new
-- install, schema.sql already creates both tables in the SAME migrate.js run, tracked as one
-- migration ("schema.sql"), so this file — a separate entry in the SAME run right after it —
-- would otherwise immediately fail with "There is already an object named 'stock_vouchers'"
-- (confirmed against a throwaway fresh database).

IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'stock_vouchers' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'stock_voucher_lines' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
