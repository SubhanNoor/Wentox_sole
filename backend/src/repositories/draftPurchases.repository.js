...
async function insertDraft(transaction, draft) {
  ...
  const systemNo = draft.system_no
    ?? await nextSequenceValue(transaction, 'dbo.seq_purchase_no');
  const request = requestWithParams(transaction, {
    ...
    // unposting a purchase back into a draft (purchases.service.js#unconfirm) supplies the
    // purchase's own existing number here instead, so it survives the round trip unchanged.
    systemNo: { type: sql.Int, value: systemNo },
  });

  const result = await request.query(`
    INSERT INTO dbo.draft_purchases (purchase_date, vendor_id, bill_no, remarks, total_value, created_by, system_no)
    OUTPUT inserted.draft_id
    VALUES (@purchaseDate, @vendorId, @billNo, @remarks, @totalValue, @createdBy, @systemNo)
  `);
