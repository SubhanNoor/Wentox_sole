...
async function insertDraftItems(transaction, draftId, items) {
  for (const [index, item] of items.entries()) {
    ...
    await request.query(`
      INSERT INTO dbo.draft_sale_return_items (
...
async function findById(draftId) {
  const draftResult = await query(
    `SELECT * FROM dbo.draft_sale_returns WHERE draft_id = @draftId`,
    { draftId: { type: sql.Int, value: draftId } },
  );
  ...
  const itemsResult = await query(
    `SELECT
     ...
     FROM dbo.draft_sale_return_items dsri
     JOIN dbo.article_colors ac ON ac.variant_id = dsri.variant_id
     JOIN dbo.articles a ON a.article_id = ac.article_id
     WHERE dsri.draft_id = @draftId
     ORDER BY dsri.line_no`,
    { draftId: { type: sql.Int, value: draftId } },
  );
...
async function list(filters = {}) {
  ...
  const result = await query(
    `SELECT * FROM dbo.draft_sale_returns ${where} ORDER BY return_date DESC, draft_id DESC`,
    params,
  );
