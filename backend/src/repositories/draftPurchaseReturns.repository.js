...
async function insertDraftItems(transaction, draftId, items) {
  for (const [index, item] of items.entries()) {
    ...
    await request.query(`
      INSERT INTO dbo.draft_purchase_return_items (
...
async function findById(draftId) {
  const draftResult = await query(
    `SELECT * FROM dbo.draft_purchase_returns WHERE draft_id = @draftId`,
    { draftId: { type: sql.Int, value: draftId } },
  );
  ...
  const itemsResult = await query(
    `SELECT dpri.*, m.name AS material_name
     FROM dbo.draft_purchase_return_items dpri
     JOIN dbo.materials m ON m.material_id = dpri.material_id
     WHERE dpri.draft_id = @draftId
     ORDER BY dpri.line_no`,
    { draftId: { type: sql.Int, value: draftId } },
  );
...
async function list(filters = {}) {
  ...
  const result = await query(
    `SELECT * FROM dbo.draft_purchase_returns ${where} ORDER BY return_date DESC, draft_id DESC`,
    params,
  );
...
async function deleteDraft(transaction, draftId) {
  ...
  await request.query('DELETE FROM dbo.draft_purchase_returns WHERE draft_id = @draftId');
}
...
async function updateDraftHeader(transaction, draftId, draft) {
  ...
  await request.query(`
    UPDATE dbo.draft_purchase_returns SET
...
async function deleteDraftItems(transaction, draftId) {
  const request = requestWithParams(transaction, { draftId: { type: sql.Int, value: draftId } });
  await request.query('DELETE FROM dbo.draft_purchase_return_items WHERE draft_id = @draftId');
}
