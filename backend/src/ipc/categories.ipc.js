...
module.exports = function register() {
  ipcMain.handle('categories:list', wrap((payload) => {
    requireSession();
    return service.list(payload);
  }));

  ipcMain.handle('categories:get', wrap((payload) => {
    requireSession();
    return service.getById(payload.id);
  }));

  ipcMain.handle('categories:create', wrap((payload) => {
    requireSession();
    return service.create(payload);
  }));

  ipcMain.handle('categories:update', wrap((payload) => {
    requireSession();
    return service.update(payload.id, payload);
  }));

  ipcMain.handle('categories:remove', wrap((payload) => {
    requireSession();
