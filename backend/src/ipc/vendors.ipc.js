...
module.exports = function register() {
  ipcMain.handle('vendors:list', wrap((payload) => {
    requireSession();
    return service.list(payload);
  }));

  ipcMain.handle('vendors:get', wrap((payload) => {
    requireSession();
    return service.getById(payload.id);
  }));

  ipcMain.handle('vendors:create', wrap((payload) => {
    requireSession();
    return service.create(payload);
  }));

  ipcMain.handle('vendors:update', wrap((payload) => {
    requireSession();
    return service.update(payload.id, payload);
  }));

  ipcMain.handle('vendors:remove', wrap((payload) => {
    requireSession();
    return service.remove(payload.id);
  }));

  ipcMain.handle('vendors:reactivate', wrap((payload) => {
    requireSession();
