// IPC layer: registers ipcMain.handle channels for receipt vouchers (RJ-03) — no business logic,
// no SQL. Entry LINES are created/edited/deleted through the existing 'receipts:*' channels with a
// voucher_id on the payload; this feature owns the header and the whole-voucher operations.
const { ipcMain } = require('electron');
const service = require('../services/receiptVouchers.service');
const authService = require('../services/auth.service');
const { wrap } = require('./wrap');
const { requireSession } = require('./session');

module.exports = function register() {
  ipcMain.handle('receipt-vouchers:list', wrap((payload) => {
    requireSession();
    return service.list(payload);
  }));

  ipcMain.handle('receipt-vouchers:get', wrap((payload) => {
    requireSession();
  ...
  ipcMain.handle('receipt-vouchers:update', wrap((payload) => {
    const session = requireSession();
  ...
  ipcMain.handle('receipt-vouchers:unpost', wrap((payload) => {
    const session = requireSession();
    return service.unpost(payload.id, session);
  }));

  // Password required, matching 'receipts:remove' (RJ-06) — this deletes the header AND every line
